...
/**
 * Ascension Level 29: Luck decreased - Relic probability adjusted
 * 운이 감소합니다.
 *
 * 상세효과:
 * 유물 확률 조정
...
public class Level29 {
    ...
    private static final Logger logger = LogManager.getLogger(Level29.class.getName());

    /**
     * Adjust relic probabilities for all dungeons
     */
    private static void adjustRelicProbabilities() {
        if (!AbstractDungeon.isAscensionMode || AbstractDungeon.ascensionLevel < 29) {
            return;
        }
        ...
        try {
            ...
            logger.info("Ascension 29: Relic probabilities adjusted to Common 50%, Uncommon 35%, Rare 15%");
        } catch (Exception e) {
